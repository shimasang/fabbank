...
var joinChannel = function(channelName, peers, username, org) {
	...
	return helper.getOrgAdmin(org).then((admin) => {
		...
		tx_id = client.newTransactionID();
		logger.debug(util.format('tx_id: "%s"', JSON.stringify(tx_id)));
		var request = {
		...
		eventhubs.forEach((eh) => {
			let txPromise = new Promise((resolve, reject) => {
				let handle = setTimeout(reject, parseInt(eventWaitTime));
		...
		logger.info('send a join channel request')
