...
logger.info("set up the client and channel objects for each org");

var initHelper = function () {
	for (let key in ORGS) {
		if (key.indexOf('org') === 0) {
			logger.debug(util.format('instantiate "%s"', key));
			let client = new hfc();
			...
			logger.debug('channel: ' + hfc.getConfigSetting('channelName'));
...
//		setupAnotherChannel(ORGS[org].peers[key]['channels'], peer, client);
	}
}

function setupAnotherChannel(channelNames, peer, client) {
		...
		return client.getUserContext(username, true).then((user) => {
			if (user && user.isEnrolled()) {
				...
				return getAdminUser(userOrg).then(function(adminUserObj) {
					...
					logger.debug(username + ' registered successfully' + enrollmentSecret);
					return caClient.enroll({
						enrollmentID: username,
						enrollmentSecret: secret
					});
