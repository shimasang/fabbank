...
var instantiateChaincode = function(targetPeers, channelName, chaincodeName, chaincodeVersion, functionName, args, username, org) {
	logger.debug('\n============ Instantiate chaincode on organization ' + org +
		' ============\n');

	var channel = helper.getChannelForOrg(org);
	var client = helper.getClientForOrg(org);
	var eventPeer = targetPeers[0];

	return helper.getOrgAdmin(org).then((user) => {
		// read the config block from the orderer for the channel
		// and initialize the verify MSPs based on the participating
		// organizations
		logger.debug('Remove unneccesarly peers');
		var removePeers = [];
		for(let peer of channel.getPeers()) {
			logger.debug(peer.getName() + ' is in the channel');
			if( targetPeers.indexOf(peer.getName()) < 0 ) {
				logger.debug(util.format('"%s" is not in the target list.', peer.getName()));
				removePeers.push(peer);
			}
		}

		for(let peer of removePeers) {
		...
		logger.debug(proposalResponses.length + ' proposal response(s)');
		...
		if (all_good) {
			...
			let data = fs.readFileSync(path.join(__dirname, ORGS[org].peers[eventPeer][
				'tls_cacerts'
			]));
			eh.setPeerAddr(ORGS[org].peers[eventPeer]['events'], {
				pem: Buffer.from(data).toString(),
				'ssl-target-name-override': ORGS[org].peers[eventPeer]['server-hostname']
			});
			eh.connect();
